...
def translate_sentence(src):
    ...
    if src in EXACT_TRANSLATIONS:
        return apply_glossary(EXACT_TRANSLATIONS[src])
    ...
    s = re.sub(r"\bSiliconians\b", "Siliconianos", s)
    s = re.sub(r"\bSiliconian\b", "Siliconiano", s)
    s = re.sub(r"\bSilconian\b", "Silconiano", s)
...
for entry in entries:
    ...
    translated = translate_sentence(src)
