...
cl %FLAGS% %INC% "%PROJ%\RecompiledFuncsTraced\funcs_*.c" /Fo"%PROJ%\build\objv\\" >nul
if errorlevel 1 exit /b 1
...
cl /nologo /c /O2 /std:c++17 /EHsc /MP /DRECOMP_POLLING %WPJ2_BUILD_MODE% %WPJ2_TRACE_MODE% %INC% ^
   ...
   "%PROJ%\runtime\rsp_native.cpp" "%PROJ%\analysis\rsp_audio_recompiled.cpp" /Fo"%PROJ%\build\objv\\"
if errorlevel 1 exit /b 1
