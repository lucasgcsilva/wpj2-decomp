 ...
 * Identidade da ROM.
 *
 * O CRC1 lido em 0x10 e apenas um campo DENTRO do arquivo: uma imagem
 * adulterada pode perfeitamente carregar o mesmo valor. Para a garantia que
 * queremos -- "qualquer ROM que bata com o nosso MD5 funciona no projeto" --
 * o hash tem de ser calculado sobre o conteudo inteiro que acabamos de ler.
 * Implementacao local e sem estado para nao acrescentar dependencia externa
 * nem um novo arquivo aos scripts de build.
 *
 * >>> PROVISORIO -- A REMOVER <<<
 * A amarracao a UM md5 e andaime de desenvolvimento, nao objetivo do projeto.
 * O alvo final e rodar outras imagens do mesmo jogo (a japonesa original, e
 * outras traducoes), e nesse mundo um md5 fixo passa a ser um estorvo.
 * Quando a identificacao virar dinamica -- por CRC1/tamanho, com os
 * deslocamentos de texto e recurso resolvidos por perfil de ROM em vez de
 * constantes -- este bloco inteiro (md5_hex, WPJ2_ROM_MD5 e o aviso em
 * load_rom) sai. Ate la ele so evita diagnostico confuso com imagem errada.
 * ------------------------------------------------------------------------ */
#define WPJ2_ROM_MD5 "3c02f56dd7b1a06be83a7a288755612f"

static void md5_hex(const uint8_t* dados, size_t n, char saida[33]) {
    ...
    for (size_t off = 0; off < total; off += 64u) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            if (pos < n)               bloco[i] = dados[pos];
            else if (pos == n)         bloco[i] = 0x80u;
            else if (pos < total - 8u) bloco[i] = 0u;
            else bloco[i] = (uint8_t)(bits >> (8u * (unsigned)(pos - (total - 8u))));
        }
        uint32_t M[16];
        for (int i = 0; i < 16; i++)
            M[i] = (uint32_t)bloco[i*4] | ((uint32_t)bloco[i*4+1] << 8) |
                   ((uint32_t)bloco[i*4+2] << 16) | ((uint32_t)bloco[i*4+3] << 24);
    ...
    for (int i = 0; i < 16; i++)
        sprintf(saida + i*2, "%02x", (unsigned)((h[i/4] >> (8*(i%4))) & 0xFFu));
    saida[32] = '\0';
}
...
static int load_rom(const char* path) {
    ...
    /* A imagem inteira ja esta em g_rom; o hash conclui a identificacao.
       Nada aqui reabre nem escreve no arquivo -- ele foi fechado acima e o
       unico alvo de patch e a copia em RDRAM feita logo adiante. */
    char md5[33];
    md5_hex(g_rom, g_rom_size, md5);
    printf("       MD5 %s\n", md5);
    if (strcmp(md5, WPJ2_ROM_MD5) != 0) {
        printf("       AVISO: MD5 diferente do registrado para a build Ryu v1.0\n");
        printf("              (esperado %s)\n", WPJ2_ROM_MD5);
        printf("              o projeto so e validado contra essa imagem;\n");
        printf("              deslocamentos de texto e recursos podem nao bater.\n");
    } else if (crc1 != 0x4F1E88F7u) {
        printf("       AVISO: CRC1 do cabecalho inconsistente com o MD5 esperado\n");
    }
